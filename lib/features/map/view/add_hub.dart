...
import 'package:rental_app/core/resources/string.dart';
...
class AddHub extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MapHubsSupaBloc(),
      child: Builder(
        builder: (context) {
          return Scaffold(
                ...
                child: ListView(
                  children: [
                    ...
                    Padding(
                      ...
                      child: const Text(addHub,style: TextStyle(fontSize:32 ,fontWeight: FontWeight.w500),),
                    ),
                    Padding(
                      padding:  leftAndRightAndTopAndTBottomPadding(0.02,0.02,0.018,0.018),
                      child: TextFieldCustom(white, const SizedBox(),const SizedBox(), nameHub, nameController, TextInputType.name, 1,(v){},(){},false),
                    ),
                    Padding(
                      padding:  leftAndRightAndTopAndTBottomPadding(0.02,0.02,0.018,0.018),
                      child: TextFieldCustom(white, const SizedBox(),const SizedBox(), desecriptionHub, desecriptionController, TextInputType.name, 1,(v){},(){},false),
                    ),
                    Padding(
                      padding: leftAndRightAndTBottomPadding(0.02,0.02,0.02),
                      child: Row(
                        children: [
                          Container(width: screenWidth*0.76,child: TextFieldCustom(white, const SizedBox(),const SizedBox(), latLongHub, latLongController, TextInputType.name, 1,(v){},(){},false)),
                          Container(width: screenWidth*0.06,child:  IconButton(onPressed: ()async{
                            var data=await addressSuggestion(latLongController.text);
                            context.read<MapHubsSupaBloc>().add(AddToList(data));
                    ...
                    Align(
                      ...
                      child: Padding(
                        ...
                        child: ButtonCustom(0,screenWidth*0.3, screenHight*0.06, BlocBuilder<MapHubsSupaBloc,MapStates>(
                          builder:(context, state) {
                            if(state is InitalState|| state is SuccessState){
                          ...
                          BodyHubsSupa bodyHubsSupa=BodyHubsSupa(name: nameController.text,latitude: double.parse(latController.text),longitude: double.parse(longController.text),description: desecriptionController.text);
                          context.read<MapHubsSupaBloc>().add(AddHubsSupa(bodyHubsSupa));
                        }),
                      ),
                    ),
                  ],
